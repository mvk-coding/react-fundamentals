...
> The use of createElement in React compared to vanilla javascript is quite 
> interesting. It indeed shows a bit of hoe Reacts works even thought the 
> function is not used often in that way. I always thought that the way of 
> adding attributes and children was more strict, but this exercise showed 
> that the function allowed multiple combinations that had (alsmost) the same 
> result. Using an array of children in the attribute object argument is 
> evaluated more strict, as it requires an key for the children, while adding 
> childrens as additional elements is accepted without the key. 
