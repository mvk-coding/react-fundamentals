...
> Styling in JSX can be done in several ways. It is important
> to remember that css peoperties have similar names when used 
> with the style attribute, though the biggest difference is the
> use of camelCase. Furthermore, the styles attribute requires 
> an object with keys to represent the to be styled property 
> (fontStyle, backgroundColor) and the value as a string. 
> The class attribute is called className in JSX. 
> It is possible to combine the default classes and style properties,
> defined in the function for the element, with classes and styles 
> added to the element as arguments. Thoug it is important to keep in mind
> that the order given for the implementation of the default values 
> and arguments determines which class or style is shown; the value 
> of an attribute more to the right overwrites the value of that same 
> attribute if it is already present. To combine classes and styles 
> one can use the spread operator. 
